...
def main(config):
    ...
    func_vectorized = blankevoort_function_vectorized if 'blankevoort' in mode else trilinear_function_vectorized
    parameter_norms = parameter_norm(opt_result.x, ground_truth.get_params().values(), func_vectorized)
    plt.close('all')
...
def bayesian_update(result, config):
    ...
    print(f"{result['parameter_norms']}")
