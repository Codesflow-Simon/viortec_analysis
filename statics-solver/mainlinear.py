...
def main(data=dict()):
    ...
    theta_min, theta_max = data['theta_constraints']
    ...
    # Springs
    lig_springA = LinearSpring(lig_top_pointA, lig_bottom_pointA, "LigSpring", 250_000, ligament_slack_length)
    ...
    knee_vec_sym = [Symbol('F_kx'), Symbol('F_ky'), 0]
    ...
    force_vec_sym = [Symbol('F_app'),0 ,0 ]
    # force_vec_sym = [500, 0, 0]
    ...
    # Add theta constraints if they exist
    if 'theta_constraints' in data:
        equations_to_solve.extend([
            theta_val - theta_min,  # theta >= theta_min
            theta_max - theta_val   # theta <= theta_max
        ])
...
if __name__ == "__main__":
    ...
    theta_min = np.radians(1e-6)
    theta_max = np.radians(10)
    
    # Instead of using Piecewise, we'll add the constraints to the equations
    data_symbolic = {
        'femur_length': 0.1, # Distance from hip to knee
        'femur_perp': 0.03, # Perpendicular distance from hip to ligament
        'tibia_perp': 0.03, # Perpendicular (to the tibia) distance from knee to ligament
        'tibia_para': 0.02, # Distance (down the tibia) from knee to ligament
        'application_length': 0.30, # Distance(down the tibia)
        'theta': theta_sym,
        'ligament_slack_length': 0.07,
        'theta_constraints': (theta_min, theta_max)  # Store the constraints for later use
    }
